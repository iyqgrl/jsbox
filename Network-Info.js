...
function getIP() {
  ...
  let info = [];
  if (!v4 && !v6) {
    info = ['Internet might be switchin', 'Yo, refresh manually to snag that IP again'];
  } else {
...
function getNetworkInfo(retryTimes = 5, retryInterval = 1000) {
  ...
  httpMethod.get('http://ip-api.com/json').then(response => {
    ...
    if (retryTimes > 0) {
      ...
      $done({
        title: 'Oopsie!',
        content: "Can't grab the current network info, honey.\nCheck your network and try again, okay?",
...
(() => {
  ...
  setTimeout(() => {
    ...
    $done({
      title: "Timeout, yo",
      content: "Connection took too long, fam\nCheck your network and give it another shot",
      icon: 'wifi.exclamationmark',
      'icon-color': '#CB1B45',
    });    
